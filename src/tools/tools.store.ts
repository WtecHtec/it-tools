...
export const useToolStore = defineStore('tools', () => {
	...
	return {
		...
		addToolToFavorites({ tool }: { tool: MaybeRef<Tool> | any }) {
			favoriteToolsName.value.unshift(get(tool).name);
			if (favoriteToolsName.value.length >= 6) {
				favoriteToolsName.value.pop()
			}
		},

		removeToolFromFavorites({ tool }: { tool: MaybeRef<Tool> | any }) {
			favoriteToolsName.value = favoriteToolsName.value.filter(name => get(tool).name !== name);
		},

		isToolFavorite({ tool }: { tool: MaybeRef<Tool> | any }) {
